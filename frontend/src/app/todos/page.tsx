...
export default function TodosPage() {
  ...
  const [confirmDialog, setConfirmDialog] = useState<{ isOpen: boolean; id: number }>({ isOpen: false, id: 0 });
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/api/todos/${confirmDialog.id}`);
      setTodos((prev) => prev.filter((t) => t.id !== confirmDialog.id));
      success("删除成功");
    } catch {
      error("删除失败");
    }
  ...
  const handleDrop = async (e: React.DragEvent, targetIndex: number) => {
    ...
    const dragIndex = newTodos.findIndex((t) => t.id === dragItemId);
    ...
    if (dragIndex !== -1 && dragIndex !== targetIndex) {
      ...
      try {
        ...
        await api.post("/api/todos/reorder", { order: orderIds });
      } catch {
        error("排序保存失败");
        fetchTodos();
      }
                ...
                {todos.map((todo, index) => (
                  <div
                    ...
                    className={`flex items-center gap-3 rounded-xl bg-white p-3.5 shadow-sm transition-all hover:shadow-md group ${
                      dragOverIndex === index && dragItemId !== todo.id
                        ? "ring-2 ring-blue-400 ring-offset-2"
                        : ""
                    }`}
                  >
                    <svg
                      className="shrink-0 cursor-grab text-gray-300 opacity-0 transition-all group-hover:opacity-100 active:cursor-grabbing"
                    ...
                    {editingId === todo.id ? (
                      ...
                      <button
                        onClick={() => setConfirmDialog({ isOpen: true, id: todo.id })}
