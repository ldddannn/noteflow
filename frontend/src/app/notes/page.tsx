    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <a href="/notes/new" className="rounded bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">
            + 新建笔记
          </a>
        </div>
        {loading ? (
          ...
          <div className="space-y-4">
            {notes.map((note) => (
              <div key={note.id} className="rounded-lg bg-white p-4 shadow hover:shadow-md transition-shadow">
                <a href={`/notes/${note.id}`} className="text-lg font-semibold text-gray-800 hover:text-blue-600">
                  {note.title}
                </a>
                {note.tag && (
                  <span className="ml-2 rounded bg-blue-50 px-2 py-0.5 text-xs text-blue-600">{note.tag}</span>
                )}
